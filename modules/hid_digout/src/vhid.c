...
char HidWrite(hiddevice_t *dev, uint8_t report_id, void *buf, size_t size){
  int bytes_send;
  bytes_send = usb_control_msg(dev,
                               USB_TYPE_CLASS | USB_ENDPOINT_OUT,
                               USBRQ_HID_SET_REPORT,
                               (USB_HID_REPORT_TYPE_FEATURE << 8) | report_id,
                               0, buf, size+1, 5000);
  if(bytes_send<0)bytes_send=0;
  return bytes_send;
}

char HidRead(hiddevice_t *dev, uint8_t report_id, void *buf, size_t size){
  int bytes_recv;
  bytes_recv = usb_control_msg(dev, USB_TYPE_CLASS | USB_ENDPOINT_IN,
...
char HidRead(hiddevice_t *dev, uint8_t report_id, void *buf, size_t size){
  ...
  if(file != INVALID_HANDLE_VALUE){
    ...
    CloseHandle(file);
    if(res>0)return size;else return 0;
  }else{
    return 0;
  }
}
