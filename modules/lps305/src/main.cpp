...
static int L_setCompensatedOutput(lua_State *L) {
  int mode = 0;
  if(lua_gettop(L) >= 2) {
    if(lua_isnumber(L, -1)) mode = lua_tointeger(L, -1);
  }
...
static int L_connect(lua_State *L) {
  ...
  lua_newtable(L);
    ...
    lua_pushstring(L, "setCompensatedOutput");
    lua_pushcfunction(L, L_setCompensatedOutput);
    lua_rawset(L, -3);
