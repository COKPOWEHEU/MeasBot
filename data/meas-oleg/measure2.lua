-- Like measure.lua but starting from -0.1 Tesla
...
gui.wnd = gui:NewWindow(950, 650, "Measure_Hall")
...
function measure.setup()
  ...
  measure.setuptab.points.val = 100
  ...
  measure.setuptab.max_field.val = 0.09
...
function measure.startConfig(filename)
  if filename == nil then 
    ...
    filename  = "/home/user/scripts/hall_oleg/.current_settings.txt"
  end
...
function measure.addPoint(field_in)
  ...
  measure.secplot.lps305_plot:Refresh()
  measure.secplot.sr830_plot:Refresh()
  measure.secplot.sr5105_plot:Refresh()
  measure.maintab.timer.text = tostring(os.clock())
end
...
measure.resfile = assert(io.open(tostring(measure.setuptab.save_name.text), "w"))
measure.resfile:write("Umax = 9.000000  " .. "Uout = " .. tostring(sourceOfSample.tab.oVolt.value) .. '\n')
...
field_in = -(measure.max_field)
measure.addPoint(field_in)
--первый замер от -0.1 Тл к 0.1 Тл
while gui:update() do
  if field_in <= 0.02 and field_in >= -0.02 then field_in = field_in + (measure.step / 2) -- изменить
...
magnet:setField(0.000001)

-- Демонстрация окончания измерения
measure.maintab.end_label = measure.maintab:NewLabel(740, 280, "END!!!")
